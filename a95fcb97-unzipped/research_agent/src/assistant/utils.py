...
# Support both old and new DDG search package names
try:
...
def duckduckgo_search(query: str, max_results: int = 3, fetch_full_page: bool = False) -> Dict[str, List[Dict[str, str]]]:
    ...
    """
    if DDGS is None:
        print("Error: No DuckDuckGo search library available. Install with: pip install ddgs")
        return {"results": []}
    ...
    try:
        ...
        for r in search_results:
            ...
            content = r.get('body') or r.get('description') or r.get('snippet', '')
            
            if not url:
                print(f"Warning: Result missing URL, skipping: {r}")
                continue
            if not content:
        ...
        print(f"Error in DuckDuckGo search: {type(e).__name__}: {str(e)}")
        return {"results": []}
